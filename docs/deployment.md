...
1. Push the project to a GitHub repository. Its root should contain `Dockerfile`, `render.yaml`, `backend`, and `frontend`. The supplied `.gitignore` excludes `.env`, `.venv`, `node_modules`, and `work`.
...
The included [Blueprint configuration](https://render.com/docs/blueprint-spec) disables automatic deployment after later pushes. To publish an update, push the commit and use **Manual Deploy → Deploy latest commit** in the service dashboard.
