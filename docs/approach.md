...
The default `gemini-3.6-flash` has a documented free tier and successfully returned structured extractions in live testing. The earlier 2.5 Flash choice was rejected by Google for this new account; the response recommended 3.6 Flash. The initial live run and the offline baseline were evaluated on the same small regression set, but no broad model or latency comparison is claimed. The REST adapter uses a 20-second timeout, no automatic retries, and at most eight history messages. It requests schema-constrained JSON, rejects blocked or incomplete candidates, and validates every field locally. A shared cooldown limits bursts and expands after quota errors. Daily quota errors are distinguished from temporary rate limits. Failed interpretation clears state that might otherwise be confirmed accidentally. Billing is controlled by the Google project, not by the model name in code.
...
Greetings and bounded appointment-selection replies are handled locally and labeled as local routing in the decision trace. They consume no provider requests. Free-form selection language still goes through the configured interpreter; an outage never silently changes providers. Saying hello preserves the active draft while invalidating any previous confirmation, as every new message does.
